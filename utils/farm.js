...
async function inventory(client, channel, type = null) {
    ...
    await channel.send({ content: "rpg inventory" }).then(async () => {
        ...
        // Emit initial inventory data if this is not a sell or use check
        if (!type) {
            client.emit('initialInventoryCheck', message.embeds[0]);
        }
                    ...
                    if (trimmedLine.includes(item)) {
                        ...
                        if (cooldown) {
                            ...
                            if (item === "duel" && 
                                ...
                                const duelCooldown = timetoms(cooldown);
                                    client.duelManager.lastDuelTime = Date.now() - (7200000 - duelCooldown);
                                logger.info(
                                    "Farm",
                                    "Cooldowns",
                                    `Duel Cooldown: ${cooldown}`
                                );
                                } else {
                                    client.duelManager.lastDuelTime = 0;
                            logger.info(
                                "Farm",
                                "Cooldowns",
                                "Duel is ready!"
